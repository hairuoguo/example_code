...
public class Filter {
    ...
    public static List<Tweet> inTimespan(List<Tweet> tweets, Timespan timespan) {
        ...
        Instant intervalStart = timespan.getStart();
        Instant intervalEnd = timespan.getEnd();
        for (Tweet tweet: tweets){//for each tweet, test if at boundary of interval or in interval, then add to returned list if true
            Instant tweetTimestamp = tweet.getTimestamp();
            boolean isAfterOrEqualsStart = tweetTimestamp.isAfter(intervalStart) || tweetTimestamp.equals(intervalStart);
            boolean isBeforeOrEqualsEnd = tweetTimestamp.isBefore(intervalEnd) || tweetTimestamp.equals(intervalEnd);
    ...
    public static List<Tweet> containing(List<Tweet> tweets, List<String> words) {
        ...
        for (Tweet tweet: tweets){
            ...
            for (String word: words){
                String regexForWord = "(?<=(^|[\\s]))(" + word.toLowerCase() + ")(?=($|[\\s]))";
                Pattern userMention = Pattern.compile(regexForWord);
                Matcher patternMatcher = userMention.matcher(tweetText);
                if(patternMatcher.find(0)){
                    tweetsContainingWords.add(tweet);
                    break; //we break once we find a single match to be more efficient
                }
            }
        }
